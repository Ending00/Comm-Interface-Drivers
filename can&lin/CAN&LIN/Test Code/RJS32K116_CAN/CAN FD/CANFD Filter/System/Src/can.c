...
void CAN_Init(CAN_InitTypeDef *CANInitStructure)
{
    ...
    */
    ...
    CAN->CAN_ERRINFO |= (CANInitStructure->FBitRateInitStructure.F_Seg1+2)<<8;
    CAN->CAN_ERRINFO |= 1<< 15; 
...
void CAN_Receive(CAN_RxMsg *RxMessage, uint8_t CAN_WorkMode)     //只给标准格式用
{
    ...
    for(count=0; count<16; count++)
        RxMessage->Data[count] = CAN->RBUF.DATA[count];
}
