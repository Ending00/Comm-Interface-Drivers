...
void CAN_IRQHandler(void)
{
    ...
    if((CAN->CAN_CR1 & Bit8_En) == Bit8_En) //AIF
    {
        CAN->CAN_CR1 |= Bit8_En;
    }
    if((CAN->CAN_CR1 & Bit9_En) == Bit9_En) //EIF
    {
        CAN->CAN_CR1 |= Bit9_En;
        CAN->CAN_CR0 |= Bit0_En;            //Bus-Off
    }
    if((CAN->CAN_CR1 & Bit10_En) == Bit10_En) //TSIF
    {
        CAN->CAN_CR1 |= Bit10_En;
    }
    if((CAN->CAN_CR1 & Bit11_En) == Bit11_En) //TPIF
    {
        CAN->CAN_CR1 |= Bit11_En;
    }
    if((CAN->CAN_CR1 & Bit12_En) == Bit12_En) //RAFIF
    {
        CAN->CAN_CR1 |= Bit12_En;
    }
    if((CAN->CAN_CR1 & Bit13_En) == Bit13_En) //RFIF
    {
        CAN->CAN_CR1 |= Bit13_En;
    }
    if((CAN->CAN_CR1 & Bit14_En) == Bit14_En) //ROIF
    {
        CAN->CAN_CR1 |= Bit14_En;
    }
    if((CAN->CAN_CR1 & Bit16_En) == Bit16_En)   //BEIF
    {
        CAN->CAN_CR1 |= Bit16_En;
    }
    if((CAN->CAN_CR1 & Bit18_En) == Bit18_En)   //ALIF
    {
        CAN->CAN_CR1 |= Bit18_En;
    }
    if((CAN->CAN_CR1 & Bit20_En) == Bit20_En)   //EPIF
    {
        CAN->CAN_CR1 |= Bit20_En;
    }     
